 ...
 */
function openEstimatesView() {
    document.getElementById('normalView').style.display = 'none';
    document.getElementById('estimatesView').style.display = 'block';
    document.body.style.overflow = 'hidden'; // Prevent scrolling of main page
...
function attachEventListeners() {
    ...
    // Estimates View
    const backFromEstimates = document.getElementById('backFromEstimates');
    if (backFromEstimates) {
        backFromEstimates.addEventListener('click', closeEstimatesView);
    }
    ...
    // Load estimate modal (commented out - replaced by Estimates View)
    const closeLoadEstimateModal = document.getElementById('closeLoadEstimateModal');
    if (closeLoadEstimateModal) {
        closeLoadEstimateModal.addEventListener('click', () => closeModal('loadEstimateModal'));
    }
    const cancelLoadButton = document.getElementById('cancelLoadButton');
    if (cancelLoadButton) {
        cancelLoadButton.addEventListener('click', () => closeModal('loadEstimateModal'));
    }
...
function openModal(modalId) {
    ...
    // Removed - loadEstimateModal replaced by Estimates View
...
async function confirmSaveEstimate(e) {
    ...
    // Refresh estimates view if it's open
...
/* OLD LOAD ESTIMATE MODAL CODE - REPLACED BY ESTIMATES VIEW
...
async function confirmDeleteEstimate() {
    ...
    // Refresh the estimates view if it's open
    const estimatesView = document.getElementById('estimatesView');
    if (estimatesView && estimatesView.style.display !== 'none') {
        await renderEstimatesList();
    }
...
// ===========================
// Estimates View Management
// ===========================

/**
 * Render the list of estimates in the estimates view
 */
async function renderEstimatesList() {
    ...
    if (saved.length === 0) {
        container.innerHTML = `
            <div class="estimates-empty">
                ...
                <p>Create estimates in the calculator and save them to see them here.</p>
            </div>
        `;
        return;
    }
    ...
    saved.forEach((estimate, index) => {
        ...
        // Get estimate data for summary
        const estimateData = estimate.estimate_data || estimate.data;
        const totalLegs = estimateData.legs?.length || 0;
        const totalHours = estimateData.legs?.reduce((sum, leg) => {
            const hours = parseInt(leg.hours || 0);
            const minutes = parseInt(leg.minutes || 0);
            return sum + hours + (minutes / 60);
        }, 0) || 0;

        const estimateId = estimate.id || index.toString();

        estimateCard.innerHTML = `
            <div class="estimate-card-header">
                <h3 class="estimate-card-name">${estimate.name}</h3>
                <div class="estimate-card-date">${dateStr}</div>
            </div>
            <div class="estimate-card-content">
                <div class="estimate-card-details">
                    <div><strong>Flight Legs:</strong> <span>${totalLegs}</span></div>
                    <div><strong>Total Hours:</strong> <span>${totalHours.toFixed(1)}</span></div>
                </div>
            </div>
            <div class="estimate-card-actions">
                <div class="estimate-card-actions-left">
                    <button class="estimate-card-action btn-primary" data-action="load" data-estimate-id="${estimateId}" title="Open Estimate">
                    ...
                    <button class="estimate-card-action" data-action="rename" data-estimate-id="${estimateId}" title="Rename Estimate">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="16" height="16" fill="currentColor">
                            <path d="M471.6 21.7c-21.9-21.9-57.3-21.9-79.2 0L362.3 51.7l97.9 97.9 30.1-30.1c21.9-21.9 21.9-57.3 0-79.2L471.6 21.7zm-299.2 220c-6.1 6.1-10.8 13.6-13.5 21.9l-29.6 88.8c-2.9 8.6-.6 18.1 5.8 24.6s15.9 8.7 24.6 5.8l88.8-29.6c8.2-2.7 15.7-7.4 21.9-13.5L437.7 172.3 339.7 74.3 172.4 241.7zM96 64C43 64 0 107 0 160V416c0 53 43 96 96 96H352c53 0 96-43 96-96V320c0-17.7-14.3-32-32-32s-32 14.3-32 32v96c0 17.7-14.3 32-32 32H96c-17.7 0-32-14.3-32-32V160c0-17.7 14.3-32 32-32h96c17.7 0 32-14.3 32-32s-14.3-32-32-32H96z"/>
                        </svg>
                        <span>Rename</span>
                    ...
                    ${estimate.id ? `
                    <button class="estimate-card-action" data-action="share" data-estimate-id="${estimateId}" title="Share Estimate">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="16" height="16" fill="currentColor">
                ...
                <div class="estimate-card-actions-right">
                    <button class="estimate-card-action btn-danger" data-action="delete" data-estimate-id="${estimateId}" title="Delete Estimate">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="16" height="16" fill="currentColor">
                            <path d="M135.2 17.7L128 32H32C14.3 32 0 46.3 0 64S14.3 96 32 96H416c17.7 0 32-14.3 32-32s-14.3-32-32-32H320l-7.2-14.3C307.4 6.8 296.3 0 284.2 0H163.8c-12.1 0-23.2 6.8-28.6 17.7zM416 128H32L53.2 467c1.6 25.3 22.6 45 47.9 45H346.9c25.3 0 46.3-19.7 47.9-45L416 128z"/>
                        </svg>
                        <span>Delete</span>
        ...
        const buttons = estimateCard.querySelectorAll('button[data-action]');
        buttons.forEach(button => {
            const action = button.dataset.action;
            const id = button.dataset.estimateId;

            button.addEventListener('click', () => {
                switch(action) {
                    case 'load':
                        loadEstimateFromCard(id);
                        break;
                    case 'rename':
                        renameEstimateFromCard(id);
                        break;
                    case 'share':
                        shareEstimateFromCard(id);
                        break;
                    case 'delete':
                        deleteEstimateFromCard(id);
                        break;
                }
            });
        });

        // Make entire card clickable to open estimate
        estimateCard.addEventListener('click', (e) => {
            // Don't trigger card click if clicking on action buttons
            if (e.target.closest('button')) {
                return;
            }
            loadEstimateFromCard(estimateId);
        });

        // Add cursor pointer style
        estimateCard.style.cursor = 'pointer';
...
/**
 * Rename estimate from card
 */
async function renameEstimateFromCard(estimateId) {
    const saved = await getSavedEstimatesFromSource();
    const estimate = saved.find(e => (e.id || saved.indexOf(e).toString()) === estimateId);
    ...
    // Set up modal for renaming (reuse saveEstimateModal)
    state.currentEstimateId = estimate.id;
    state.currentEstimateName = estimate.name;

    document.getElementById('saveEstimateModalTitle').textContent = 'Rename Estimate';
    document.getElementById('estimateName').value = estimate.name;
    document.getElementById('confirmSaveEstimateButton').style.display = 'none';
    document.getElementById('confirmUpdateEstimateButton').style.display = 'inline-block';

    openModal('saveEstimateModal');
}

/**
 * Share estimate from card
 */
async function shareEstimateFromCard(estimateId) {
    const saved = await getSavedEstimatesFromSource();
    const estimate = saved.find(e => e.id === estimateId);
    ...
    await shareEstimateAction(estimate);
}

/**
 * Export estimate to PDF from card
 */
async function exportEstimateToPDF(estimateId) {
    const saved = await getSavedEstimatesFromSource();
    const estimate = saved.find(e => (e.id || saved.indexOf(e).toString()) === estimateId);
    ...
    // Load estimate temporarily to generate PDF
    const currentEstimateId = state.currentEstimateId;
    const currentEstimateName = state.currentEstimateName;
    const currentIsSaved = state.isSaved;
    ...
    state.currentEstimateId = currentEstimateId;
    state.currentEstimateName = currentEstimateName;
    state.isSaved = currentIsSaved;
    updateMainHeading();
}

/**
 * Delete estimate from card
 */
async function deleteEstimateFromCard(estimateId) {
    const saved = await getSavedEstimatesFromSource();
    const estimate = saved.find(e => (e.id || saved.indexOf(e).toString()) === estimateId);
